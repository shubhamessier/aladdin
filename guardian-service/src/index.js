...
import { BlockchainWriter } from './blockchain/writer.js';
...
const logger = {
    ...
    critical: (msg) => console.error(JSON.stringify(msg)),
};
const metrics = {
    emit: (data) => { },
};
...
const riskEngineClient = {
    healthCheck: async () => { },
    initializeModels: async (history) => { },
};
async function verifyContracts() {
    return {
        assetRegistry: { getActiveAssets: async () => [] },
        vault: {
            assetLedgers: async (token) => ({}),
            getOpenDerivativePositions: async () => [],
            currentCBLevel: async () => 0,
            paused: async () => false,
        },
        oracleAdapter: { getBatchPrices: async (tokens) => [] },
        strategyManager: {
            getActiveStrategies: async () => [],
            getStrategyAssets: (s) => 0,
            isStrategyActive: (s) => true,
        },
        governance: { getPendingProposals: async () => [] },
    };
}
function reconstructPortfolio(assets, balances, prices, strategyStates, positions) {
    return { totalValueUSD: 0 };
}
async function fetchPriceHistory(assets, days) {
    return [];
}
function determineState(cbLevel, paused, prices) {
    if (paused)
        return 'SHUTDOWN';
    if (cbLevel > 0)
        return 'RESTRICTED';
...
export async function bootstrap() {
    ...
    const balances = await Promise.all(assets.map(a => contracts.vault.assetLedgers(a.token)));
    const prices = await contracts.oracleAdapter.getBatchPrices(assets.map(a => a.token));
    // 3. Read strategy states
    const strategies = await contracts.strategyManager.getActiveStrategies();
    const strategyStates = await Promise.all(strategies.map(s => ({
        address: s,
...
// === Main Loop ===
export async function guardianCycle() {
    ...
    try {
        ...
        if (drift.l1Norm > regimeParams.driftThresholdBps) {
            const trades = rebalancer.generateTrades(currentAllocations, targetAllocations, portfolio, prices, { maxTradeUSD: parameters.maxTradeUSD, maxSlippageBps: parameters.maxSlippageBps });
            actions.push(...trades);
        }
