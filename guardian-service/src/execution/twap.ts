...
const oracleAdapter = {
    getPrice: async (token: string) => ({ price: 1000n * WAD }),
};

const routerSelector = {
    getBestRouter: async (tIn: string, tOut: string, amt: bigint) => '0xRouter',
};
...
export class TWAPExecutor {
    async execute(params: TWAPParams): Promise<TWAPResult> {
    ...
    private computeSlices(params: TWAPParams): Slice[] {
