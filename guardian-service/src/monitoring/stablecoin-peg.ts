...
const stablecoinConfigs = new Map<string, any>([
    ['0xUSDC', {}],
    ['0xUSDT', {}],
    ['0xDAI', {}]
]);
...
export class StablecoinPegMonitor {
    private previousPrices = new Map<string, bigint>();

    public assess(prices: Map<string, PriceData> | PriceData[]): StablecoinHealthReport {
        const reports: StablecoinReport[] = [];
        
        let priceMap: Map<string, PriceData>;
    ...
    public checkPegs(prices: PriceData[]): 'HEALTHY' | 'WATCH' | 'WARNING' | 'CRITICAL' {
        return this.assess(prices).overallHealth;
    }

    private worstStatus(reports: StablecoinReport[]): 'HEALTHY' | 'WATCH' | 'WARNING' | 'CRITICAL' {
        let worst = 0;
        ...
        return (Object.keys(levels) as any).find((key: any) => (levels as any)[key] === worst) || 'HEALTHY';
    }
}
